...
def kmer_rf_proteome_val_mydataset():
    # 1. 读取数据
    for k in [1,2,3]:
        df_mapping = pd.read_csv(r'../data/AllDataIdMapping.csv', sep='\t')
        df_kmer = pd.read_csv(f'../data/allvirusProteome{k}Kmer.csv',sep=',').rename(columns={'Unnamed: 0': 'Taxid'})
        ...
        for train_val_index, test_index in sss.split(X, y):
            ...
            # 网格搜索
            grid = GridSearchCV(RandomForestClassifier(random_state=train_val_index[0]), param_grid, cv=5, scoring='roc_auc', n_jobs=64)
...
def kmer_knn_proteome_val_mydataset():
    # 1. 读取数据
    for k in [1, 2, 3]:
...
def kmer_svm_proteome_val_mydataset():
    # 1. 读取数据
    for k in [1, 2, 3]:
        df_mapping = pd.read_csv('../data/AllDataIdMapping.csv', sep='\t')
        df_kmer = pd.read_csv(f'../data/allvirusProteome{k}Kmer.csv', sep=',').rename(columns={'Unnamed: 0': 'Taxid'})
...
def kmer_xgboost_proteome_val_mydataset():
    # 1. 读取数据
    for k in [1,2,3]:
        df_mapping = pd.read_csv('../data/AllDataIdMapping.csv', sep='\t')
        df_kmer = pd.read_csv(f'../data/allvirusProteome{k}Kmer.csv', sep=',').rename(columns={'Unnamed: 0': 'Taxid'})
...
if __name__ == "__main__":
    ...
    kmer_xgboost_proteome_val_mydataset()
