...
def ppiBinary_rf_val_mydataset():
    # 获得每个病毒不同阈值下的ppi
...
def ppiBinary_knn_val_mydataset():
    # 获得每个病毒不同阈值下的ppi
    df = pd.read_csv('../data/all_virus_ppi_matrix_threshold999.csv',index_col=0)
    df = df.groupby('taxid', as_index=False).max()
    #print(df)
...
def ppiBinary_svm_val_mydataset():
    # 获得每个病毒不同阈值下的ppi
    df = pd.read_csv('../data/all_virus_ppi_matrix_threshold999.csv',index_col=0)
    df = df.groupby('taxid', as_index=False).max()
    #print(df)
...
def ppiBinary_xgboost_val_mydataset():
    # 获得每个病毒不同阈值下的ppi
    df = pd.read_csv('../data/all_virus_ppi_matrix_threshold999.csv',index_col=0)
    df = df.groupby('taxid', as_index=False).max()
    #print(df)
