...
def ppiBinary_rf_test_paperdataset():
    ...
    for top_num in [300]:
        ...
        result_df.to_csv(f"../result/test/ppi/rf_top{top_num}_result.csv", index=False)
        summary.to_csv(f"../result/test/ppi/rf_top{top_num}_summary.csv")

        best_params_df = pd.DataFrame(best_params_list)
        best_params_df.to_csv(f"../result/test/ppi/rf_best_params_top{top_num}.csv", index=False)
