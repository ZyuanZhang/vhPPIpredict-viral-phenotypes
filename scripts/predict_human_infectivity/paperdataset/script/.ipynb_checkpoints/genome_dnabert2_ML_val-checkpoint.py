...
def genome_rf_dnabert2_val_paperdataset():
    # 1. 读取数据
    ...
    df_genome = pd.read_csv(f'../data/allvirusGenomeEmbedding_dnabert2.csv').rename(columns={'Unnamed: 0': 'Taxid'})
    df = pd.merge(df_mapping[['Taxid', 'Label']], df_genome, on='Taxid')
    ...
    result_df.to_csv(f"../result/val/genome/rf_result_dnabert2.csv", index=False)
    summary.to_csv(f"../result/val/genome/rf_summary_dnabert2.csv")



def genome_svm_dnabert2_val_papertest():
    # 1. 读取数据
    dt_mapping_0 = pd.read_csv("../data/AllDataIdMapping.csv", sep="\t")
    dt_mapping_0 = dt_mapping_0[dt_mapping_0["belong"]=="861paper"].reset_index(drop=True)
    target_taxid = list(dt_mapping_0["Taxid"])
    ...
    # 保存文件
    os.makedirs("../result/val/genome", exist_ok=True)
    result_df.to_csv(f"../result/val/genome/svm_result_dnabert2.csv", index=False)
    summary.to_csv(f"../result/val/genome/svm_summary_dnabert2.csv")



def genome_knn_dnabert2_val_papertest():
    # 1. 读取数据
    dt_mapping_0 = pd.read_csv("../data/AllDataIdMapping.csv", sep="\t")
    dt_mapping_0 = dt_mapping_0[dt_mapping_0["belong"]=="861paper"].reset_index(drop=True)
    target_taxid = list(dt_mapping_0["Taxid"])
    ...
    os.makedirs("../result/val/genome", exist_ok=True)
    top_num = X.shape[1]  # 特征数
    result_df.to_csv(f"../result/val/genome/knn_result_dnabert2.csv", index=False)
    summary.to_csv(f"../result/val/genome/knn_summary_dnabert2.csv")


def genome_xgboost_dnabert2_val_papertest():
    # 1. 读取数据
    dt_mapping_0 = pd.read_csv("../data/AllDataIdMapping.csv", sep="\t")
    dt_mapping_0 = dt_mapping_0[dt_mapping_0["belong"]=="861paper"].reset_index(drop=True)
    target_taxid = list(dt_mapping_0["Taxid"])
    ...
    # 保存文件
    os.makedirs("../result/val/genome", exist_ok=True)
    result_df.to_csv(f"../result/val/genome/xgboost_result_dnabert2.csv", index=False)
    summary.to_csv(f"../result/val/genome/xgboost_summary_dnabert2.csv")
...
if __name__ == "__main__":
    ...
    genome_rf_dnabert2_val_papertest()
