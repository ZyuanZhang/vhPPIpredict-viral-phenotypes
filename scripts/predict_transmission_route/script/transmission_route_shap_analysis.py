...
def shap_analysis_transmission_route():
    # ========= 1. 加载数据 =========
    ...
    # ========= 2. 特征选择 =========
    top_num = 350
    feature_importances_df = pd.read_csv("./data/rf_feature_importances_100split_Trprimary.csv")
    ...
    shap_importance.to_csv("./result/transmission_route_shap_importance_svm.csv", index=False)

    # ========= 6. 可选: 可视化 top 特征 =========
    shap.summary_plot(shap_values, X, feature_names=top_features, class_names=classes, show=False)
    plt.tight_layout()
    plt.savefig("./result/transmission_route_shap_analysis.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    shap_analysis_transmission_route()
