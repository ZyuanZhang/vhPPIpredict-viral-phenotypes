...
def splitKmer(protseq, k=3):
    """计算k-mer组成"""
    return Counter(protseq[i:i+k] for i in range(len(protseq) - k + 1))
...
def getMatrixKmer(k=1):
    ...
    """构建k-mer特征矩阵"""
    dt_vproteome = dt_hpv_subspecies.drop_duplicates(subset=["virus_name","virus_unid","label"], keep="first", ignore_index=True)
    virus_ids = set(dt_vproteome["virus_unid"])

    # 多进程加载序列并计算k-mer
    virus_kmer = load_sequences_parallel(virus_ids, "/data/150T/databases/help_zhangzhiyuan/PredictAllHumanVirusPpiDatasetEmbedCase1/virusProtSeq/", k, n_processes=20)

    # 所有k-mer组合
    amino_acids = "ACDEFGHIKLMNPQRSTUVWXY"
    kmer_combinations = [''.join(p) for p in product(amino_acids, repeat=k)]
    
    # 初始化特征矩阵
    row_ids = sorted(dt_vproteome["virus_name"].unique())
    feat_matrix = pd.DataFrame(0, index=row_ids, columns=kmer_combinations + ["risk_label"])

    # 填充特征矩阵
    for _, row in dt_vproteome.iterrows():
        vname = row["virus_name"]
        if row["virus_unid"] in virus_kmer:
            for kmer, count in virus_kmer[row["virus_unid"]].items():
                feat_matrix.at[vname, kmer] += count
        feat_matrix.loc[vname, "risk_label"] = row["label"]
    
    print(feat_matrix.shape)
    feat_matrix.to_csv(f"../data/hpv_proteome_{k}mer.csv")
